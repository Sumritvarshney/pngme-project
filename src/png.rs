...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        let header_bytes: &[u8] = &iter[..8];
        if Self::STANDARD_HEADER != header_bytes {
            return Err(Error::new(ErrorKind::InvalidData, "Invalid header"));
        }

        iter = &iter[8..]; // Move past the header

        while iter.len() >= 12 {
            let length_bytes: &[u8] = &iter[..4];
            let length = u32::from_be_bytes(length_bytes.try_into().map_err(|_| {
                Error::new(ErrorKind::InvalidData, "Failed to read chunk length")
            })?);

            if iter.len() < (length as usize + 12) {
                return Err(Error::new(ErrorKind::InvalidData, "Not enough data for chunk"));
            }

            let chunk_data = &iter[0..(length as usize + 12)];
            let chunk = Chunk::try_from(chunk_data)?;
            chunks.push(chunk);

            // Advance the iterator
            iter = &iter[(length as usize + 12)..];
        }
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, Error> {
        if let Some(position) = self.chunks.iter().position(|c| c.chunk_type().to_string() == chunk_type) {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut result_bytes = Vec::with_capacity(8 + self.chunks.iter().map(|chunk| chunk.as_bytes().len()).sum::<usize>());
...
fn chunk_from_strings(chunk_type: &str, data: &str) -> Result<Chunk, Error> {
    // Attempt to parse the chunk type from the string
    let chunk_type = ChunkType::from_str(chunk_type)
        .map_err(|_| Error::new(ErrorKind::InvalidInput, "Invalid chunk type"))?;
    
    // Convert the data string to a Vec<u8>
